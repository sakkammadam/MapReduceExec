#ifndef MRHOPE_FILEPROCESSORBASE_HPP
#define MRHOPE_FILEPROCESSORBASE_HPP
...
private:
    ...
    std::map<std::string, std::vector<std::vector<std::vector<std::tuple<std::string, int, int>>>>> mapperRawOutput;
...
public:
    ...
    void setRawMapperOutput(const std::map<std::string, std::vector<std::vector<std::vector<std::tuple<std::string, int, int>>>>> &raw_mapper_data){
        this->mapperRawOutput = raw_mapper_data;
    }
    // Getter - get the raw mapper output data
    std::map<std::string, std::vector<std::vector<std::vector<std::tuple<std::string, int, int>>>>> getRawMapperOutput(){
...
#endif //MRHOPE_FILEPROCESSORBASE_HPP